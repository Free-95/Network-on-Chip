...
module tb_uart_standalone;
    ...
    // =========================================================================
    // Task: wait until the scoreboard has processed all expected bytes
    // =========================================================================
    task automatic wait_rx (input int n_bytes);
        int timeout;
        timeout = n_bytes * (`SIM_CPB * 15);  // 15 bit-times per byte max
        
        // Instead of counting pulses, just wait for the read pointer to catch up 
        // to the write pointer in our scoreboard
        ...
        if (timeout == 0)
            $display("  [WARN] wait_rx(%0d) timed out! exp_wr=%0d, exp_rd=%0d", n_bytes, exp_wr, exp_rd);
    endtask
